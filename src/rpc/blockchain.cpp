...
static UniValue getblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"tx\" : [               (array of objects) The transactions in the format of the getrawtransaction RPC. Different output from verbosity=1 \"tx\" result.\n"
            "         ,...\n"
            "  ],\n"
            "  ,...                     Same output as verbosity=1.\n"
            "}\n"
...
static UniValue verifychain(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. checklevel   (numeric, optional, default=" + strprintf("%d", nCheckLevel) + ") How thorough the block verification is (0-4).\n"
            "2. nblocks      (numeric, optional, default=" + strprintf("%d", nCheckDepth) + ") The number of blocks to check (0=All).\n"
            "\nResult:\n"
...
static UniValue getchaintxstats(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, optional) Size of the window in number of blocks. If excluded, defaults to one month.\n"
...
static UniValue getblockstats(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "  \"feerate_percentiles\": [  (array of numerics) Feerates at the 10th, 25th, 50th, 75th, and 90th percentile weight unit (in satoshis per virtual byte)\n"
