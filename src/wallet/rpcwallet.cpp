...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            "sendmany \"\" \"amounts\" ( minconf \"comment\" \"subtractfeefrom\" replaceable conf_target \"estimate_mode\")\n"
            ...
            "5. subtractfeefrom         (array, optional) A json array of addresses.\n"
...
static UniValue createwallet(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "2. disable_private_keys   (boolean, optional, default=false) Disable the possibility of private keys (only watch-onlys are possible in this mode).\n"
            "\nResult:\n"
            "{\n"
...
static UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default=<pool_address>) The bitcoin address to receive the change\n"
                            "     \"changePosition\"         (numeric, optional, default=<random>) The index of the change output\n"
                            "     \"change_type\"            (string, optional) The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\". Default is set by -changetype.\n"
                            "     \"includeWatching\"        (boolean, optional, default=false) Also select inputs which are watch only\n"
                            "     \"lockUnspents\"           (boolean, optional, default=false) Lock selected unspent outputs\n"
                            "     \"feeRate\"                (numeric, optional) Set a specific fee rate in " + CURRENCY_UNIT + "/kB. If excluded, the wallet determines the fee.\n"
...
static UniValue getaddressesbylabel(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{ (object) Bitcoin addresses, as keys of sub-objects\n"
...
UniValue walletcreatefundedpsbt(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                            ...
                            "       ,...\n"
                            "     ]\n"
                            "2. \"outputs\"               (array, required) a json array of outputs (key-value pairs)\n"
                            "   [\n"
                            "    {\n"
                            "      \"address\": x.xxx,    (object, optional) A key-value pair. The key (string) is the bitcoin address, the value (float or string) is the amount in " + CURRENCY_UNIT + "\n"
                            "    },\n"
                            "    {\n"
                            "      \"data\": \"hex\"        (object, optional) A key-value pair. The key must be \"data\", the value is hex-encoded data\n"
                            "    }\n"
